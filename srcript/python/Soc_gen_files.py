...
class Soc_Generator():
    ...
    def insert_mem_to_hash_head(self, _file, mode, keyDict, soc):
...
%s.next = &mem_%s_hash_head_%d_%d;\n""" % (hash_head, hash_head, hash_head, mode, key, 0)
            ...
            for reg in range(len(keyDict[key])):
                ...
                if reg == len(keyDict[key]) - 1:
                    ...
                    # store the last one            
                    str = """
%s.mem = (void *)&%s%s_%s_%sm;
%s.reg = NULL;
%s.next = NULL;\n""" % (item, prefix_, soc, mode.upper(), keyDict[key][reg]._regName.upper(), item, item)
                    _file.write(str)
                    break
...
%s.mem = (void *)&%s%s_%s_%sm;
%s.reg = NULL;
%s.next = &mem_%s_hash_head_%d_%d;\n""" % (item, prefix_, soc, mode.upper(), keyDict[key][reg]._regName.upper(), item, item, mode, key, reg+1)
                _file.write(str)




    def insert_reg_to_hash_head(self, _file, mode, keyDict, soc):
...
%s.mem = NULL;
%s.reg = &%s_%s_%sr;
%s.next = NULL;\n""" % (item, item, soc, mode.upper(), keyDict[key][reg]._regName.upper(), item)
                    _file.write(str)
                    break
...
%s.mem = NULL;
%s.reg = &%s_%s_%sr;
%s.next = &reg_%s_hash_head_%d_%d;\n""" % (item, item, soc, mode.upper(), keyDict[key][reg]._regName.upper(), item, mode, key, reg+1)
                _file.write(str)
    ...
    def GenerateAllModeHeaderCFile(self, objDict, soc):
        print "Generate the allmode.h File."
        
        f = file(soc + "_" + 'allmode.h', 'w')
        f.write("\n//THE FILE IS GENERATED AUTOMATICLY, DO NOT EDIT BY MANNUAL!!!\n\n")
        f.write("void %s_hash_head_init();\n" % soc)
        f.write("void %s_mode_init();\n" % soc)
        f.close()
        ...
        f = file(soc + "_" + 'allmode.c', 'w')
        f.write("\n//THE FILE IS GENERATED AUTOMATICLY, DO NOT EDIT BY MANNUAL!!!\n\n")
        f.write("#include <stdio.h>\n")
        f.write("#include \"common.h\"\n")
        f.write("#include \"%s_allmode.h\"\n" % soc)
        f.write("#include \"cli_view.h\"\n")
        f.write("#include \"%s_allfield.h\"\n" % soc)
        f.write("#include \"%s_allreg.h\"\n" % soc)
        f.write("#include \"%s_allmem.h\"\n" % soc)
        f.write("#include \"%s_table_depth.h\"\n" % soc)
        

        f.write("\nextern void clean_mem(unsigned int base, unsigned int offset, int bits, int depth, char *name, char *sheetName, char *table_type);\n")
        
        f.write("\nmode_info_t %s_mode_list[MAX_MODE];\n\n" % soc) # define the mode_list
        ...
        for mode in objDict:            
            ...
            if objDict[mode][1][0] > 0:                
                ...
                str = self.insert_reg_to_hash_head(f, mode, keyDict, soc) # insert the reg into the hash entry
            ...
            if objDict[mode][2][0] > 0:                
                ...
                str = self.insert_mem_to_hash_head(f, mode, keyDict, soc) # insert the mem into the hash entry
        ...
        for mode in objDict:
            if objDict[mode][1][0] > 0 and objDict[mode][2][0] > 0 :
                mode_id = "%s_%s_MODE" % (soc.upper(), mode.upper())
            ...
            str = """
    %s_mode_list[%s_%s_MODE].name = \"%s\";
    %s_mode_list[%s_%s_MODE].nKey_r = %d;
    %s_mode_list[%s_%s_MODE].nKey_m = %d;
    %s_mode_list[%s_%s_MODE].mode_id = %s;
    %s_mode_list[%s_%s_MODE].rHead = %s;
    %s_mode_list[%s_%s_MODE].mHead = %s;
"""% (soc, soc.upper(), mode.upper(), mode.lower(), soc, soc.upper(), mode.upper(), nKey_r, soc, soc.upper(), mode.upper(), nKey_m, soc, soc.upper(), mode.upper(), mode_id, soc, soc.upper(), mode.upper(), reg_hash_head, soc, soc.upper(), mode.upper(), mem_hash_head)
        ...
        f.write("\n\nint %s_all_table_init(int module_id, int argc, char *args[])\n{\n" % soc)
    ...
    def GenerateAllRegFile(self, objDict, soc):
        print "Generate the allreg.h File."
        f = file(soc + "_" + 'allreg.h', 'w')
        f.write("\n//THE FILE IS GENERATED AUTOMATICLY, DO NOT EDIT BY MANNUAL!!!\n\n")
        ...
        for mode in objDict:            
            if objDict[mode][1][0] > 0:
                regList = objDict[mode][1][1]
                for reg in range(len(regList)):
                    f.write("extern field_info_t %s_%s_%sf[];\n" %(soc, mode.lower(), (regList[reg]._regName).lower()))
        ...
        for mode in objDict:            
            if objDict[mode][1][0] > 0:
                ...
                for reg in regList:
                    str = "reg_info_t %s_%s_%sr  = {\"%s\",\"%s\", %s, 0x%x, %d, %s_%s_%sf};\n" % (soc, mode.upper(), reg._regName.upper(), reg._regName, reg._regType, reg._regBaseAddr, reg._regOffsetAddr, len(reg._regFieldList), soc, mode.lower(), (reg._regName).lower())
                    f.write(str)
    ...
    def GenerateAllMemFile(self, objDict, soc):
        print "Generate the allmem.h File."
        f = file(soc + "_" + 'allmem.h', 'w')
        f.write("\n//THE FILE IS GENERATED AUTOMATICLY, DO NOT EDIT BY MANNUAL!!!\n\n")

        f.write("\n#include \"%s_table_depth.h\" \n\n" % soc)
        ...
        for struct_name in key_type_field_info_dict:            
            f.write("extern field_info_t %s_%s_keytype_f;\n" %(soc, (struct_name).lower()))  
                ...
                for reg in range(len(regList)):
                    ...
                    if nKeyType == 1:
                        ...
                        for info in regList[reg]._regFieldList:
                            key = info[0]
                            f.write("extern field_info_t %s_%s_%s_%sf[];\n" %(soc, mode.lower(), (regList[reg]._regName).lower(), key))
            ...
            if objDict[mode][2][0] > 0:
                ...
                for reg in range(len(regList)):
                    ...
                    if nKeyType == 1:
                        ...
                        str = "mem_info_t %s_%s_%sm  = { \"%s\", \"%s\", %s, 0x%x, %d, %d, %s, %s_%s_%sf, %s, NULL};\n" % (soc, mode.upper(), regList[reg]._regName.upper(), regList[reg]._regName, regList[reg].regType, regList[reg]._regBaseAddr, regList[reg]._regOffsetAddr,   regList[reg]._regFieldList[0][2], len(regList[reg]._regFieldList[0][1]), key, soc, mode.lower(), (regList[reg]._regName).lower(), regList[reg]._table_struct_name.upper())

                        f.write(str)
                    else :
                        for info in regList[reg]._regFieldList:
                            key = info[0]                            
                            str = "mem_info_t %s_%s_%s_%sm  = { \"%s\", \"%s\", %s, 0x%x, %d, %d, %s,  %s_%s_%s_%sf, %s, &%s_%s_keytype_f};\n" % (soc, mode.upper(), regList[reg]._regName.upper(), key, regList[reg]._regName, regList[reg].regType, regList[reg]._regBaseAddr, regList[reg]._regOffsetAddr, info[2], len(info[1]), key, soc, mode.lower(), (regList[reg]._regName).lower(), key, regList[reg]._table_struct_name.upper(), soc, (regList[reg]._table_struct_name).lower())
                            f.write(str)
            ...
            if objDict[mode][2][0] > 0:
                ...
                for reg in range(len(regList)):
                    ...
                    if table_type in ["HASH", "TCAM"]:                                                
                        ...
                        nKeyType = len(regList[reg]._regFieldList)
                        if nKeyType == 1:                            
                            str = "hash_tcam_mem_info_t  hash_tcam_%s_%s_%sm = {%d, %d, &%s_%s_%sm, &%s_%s_%sm};\n" % (soc, mode.upper(), regList[reg]._regName.upper(), max_width_data, max_width_key, soc, mode.upper(), regList[reg]._regName.upper(), soc, mode.upper(), regList[1]._regName.upper())

                            f.write(str)
                        else :
                            for info in regList[reg]._regFieldList:
                                key = info[0]                                
                                str = "hash_tcam_mem_info_t hash_tcam_%s_%s_%s_%sm  = {%d, %d, &%s_%s_%s_%sm,  &%s_%s_%s_%sm};\n" % (soc, mode.upper(), regList[reg]._regName.upper(), key, max_width_data, max_width_key, soc, mode.upper(), regList[reg]._regName.upper(), key, soc, mode.upper(), regList[1]._regName.upper(), key)
                                f.write(str)
                    if table_type in ["HASH", "TCAM"]:
                        break
    ...
    def GenerateAllFieldFile(self, objDict, soc):
        print "Generate the allfield.h File."
        f = file(soc + "_" + 'allfield.h', 'w')
        f.write("\n//THE FILE IS GENERATED AUTOMATICLY, DO NOT EDIT BY MANNUAL!!!\n\n")
        ...
        for struct_name in key_type_field_info_dict:            
            ...
            f.write("field_info_t %s_%s_keytype_f = {\"key_type\", %d, %d, 0};\n" %(soc, struct_name.lower(), len_, bp))            
                ...
                for reg in range(len(regList)):
                    ...
                    if nKeyType == 1:
                        ...
                        for info in fieldInfoList :
                            ...
                            f.write("field_info_t %s_%s_%s_%sf[] = \n{\n" %(soc, mode.lower(), regList[reg]._regName.lower(), key))
    ...
    def GenerateCli_Mode_Mode_H(self, allObjDict, memObjDict, soc):
        ...
        f = file(soc + "_" + "cli_mode.h", 'w')
        f.write("\n//THE FILE IS GENERATED AUTOMATICLY, DO NOT EDIT BY MANNUAL!!!\n\n")
        #f.write("extern struct cli_tree *ctree;\n")
        
        str = """\n
int cli_%s_register_mode(int module_id, int argc, char *args[])
{
	ctree->mode = %s_REGISTER_MODE;
	setPrompt("GalaxyWind(config/%s/register)#");
	printf("func: cli_%s_register_mode excuted!\\n");
	return 0;
}

int cli_%s_register_exit(int module_id, int argc, char *args[])
{
	ctree->mode = %s_MODE;
	setPrompt("GalaxyWind(config/%s)#");
	printf("func: cli_%s_register_exit excuted!\\n");
	return 0;
}

int cli_%s_table_mode(int module_id, int argc, char *args[])
{
	ctree->mode = %s_TABLE_MODE;
	setPrompt("GalaxyWind(config/%s/table)#");
	printf("func: cli_%s_table_mode excuted!\\n");
	return 0;
}

int cli_%s_table_exit(int module_id, int argc, char *args[])
{
	ctree->mode = %s_MODE;
	setPrompt("GalaxyWind(config/%s)#");
	printf("func: cli_%s_table_exit excuted!\\n");
	return 0;
}\n\n""" % (soc, soc.upper(), soc, soc,     soc, soc.upper(), soc, soc,     soc, soc.upper(), soc, soc,     soc, soc.upper(), soc, soc )
        f.write(str)
        ...
        for mode in allObjDict:
            if mode in memObjDict:
                exit_mode = "%s_TABLE_MODE" % soc.upper()
                prefix = "table"
            else :
                exit_mode = "%s_REGISTER_MODE" % soc.upper()
                prefix = "register"
                
            str = """\n
int cli_%s_%s_mode(int module_id, int argc, char *args[])
{
	ctree->mode = %s_%s_MODE;
	setPrompt("GalaxyWind(config/%s/%s)#");
	printf("func: cli_%s_%s_mode excuted!\\n");
	return 0;
}

int cli_%s_%s_exit(int module_id, int argc, char *args[])
{
	ctree->mode = %s;
	setPrompt("GalaxyWind(config/%s/%s)#");
	printf("func: cli_%s_%s_exit excuted!\\n");
	return 0;
}\n\n""" % (soc, mode.lower(), soc.upper(), mode.upper(),soc, mode.lower(), soc, mode.lower(), soc, mode.lower(),exit_mode,soc, prefix, soc, mode.lower())
...
<root>
	<view name="%s_REGISTER_MODE">\n""" % soc.upper()
		...
		<command type="normal">
			...
			<funcName type="user">cli_%s_%s_mode</funcName>
			<newMode>%s</newMode>
		</command>\n""" % (mode.lower(), mode.lower(), soc, mode.lower(), mode.upper())
        ...
        <command type="normal">
            ...
            <funcName type="user">cli_%s_register_exit</funcName>
            <newMode></newMode>
        </command>\n	
    </view>
</root>\n""" % soc
    ...
    def GenerateTableXMLFile(self, allObjDict, soc): # give the table obj
        print "Generate the table.xml and socxxx.xml File."

        # add the new mode for tables
        f = file(soc + "_" + "table.xml", 'w')
        str = """<?xml version="1.0" encoding="utf-8"?>
<!--
	Description:
                THE FILE IS GENERATED AUTOMATICLY, DO NOT EDIT BY MANNUAL!!!
...
<root>
	<view name="%s_TABLE_MODE">\n""" % soc.upper()
        ...
        <command type="normal">
            ...
            <funcName type="user">cli_%s_set_table_pool_mode</funcName>
            <newMode></newMode>
        </command>\n""" % soc
        f.write(str)
        ...
        <command type="normal">
            ...
            <funcName type="user">%s_all_table_init</funcName>
            <newMode></newMode>
        </command>\n""" % soc
        f.write(str)
        ...
        <command type="normal">
            ...
            <funcName type="user">cli_%s_table_exit</funcName>
            <newMode></newMode>
        </command>\n	
    </view>
</root>\n""" % soc
    ...
    def GenerateRegMemXMLFile_For_TCAM_st_l3_defip(self, allObjDict, mode, soc):
...
<root>"""
            ...
            str = """
    <view name="%s_%s_MODE">\n""" %(soc, mode.upper()) # Here the mode name should be upper.
            f.write(str)
            ...
            if allObjDict[mode][1][0] > 0:
                ...
                for reg in range(allObjDict[mode][1][0]):                    
                    ...
                    str = "            </help>\n"
                    str+= "            <funcName type=\"user\">cli_%s_%s_%s_set_reg</funcName>\n" % (soc, mode, regList[reg]._regName)
                    str+= "            <newMode></newMode>\n        </command>\n\n"
                    f.write(str)
                    ...
                    str+= "                <helpInfo>The register name.</helpInfo>\n"            
                    str+= "            </help>\n"
                    str+= "            <funcName type=\"user\">cli_%s_%s_%s_get_reg</funcName>\n" % (soc, mode, regList[reg]._regName)
                    str+= "            <newMode></newMode>\n        </command>\n\n"
                    f.write(str)
            ...
            if allObjDict[mode][2][0] > 0:
                ...
                if table_type == "TCAM":                       
                    ...
                    str+= "            </help>\n"
                    str+= "            <funcName type=\"user\">cli_tcam_init_%s_%s</funcName>\n" % (soc, mode)
                    str+= "            <newMode></newMode>\n        </command>\n\n"
                    f.write(str)
                ...
                for info in fieldInfoList :
                    ...
                    for i in range(2):
                        ...
                        str = "            </help>\n"
                        str+= "            <funcName type=\"user\">cli_%s_%s_%s_%s_set_mem_%s</funcName>\n" % (soc, mode, regList[0]._regName, key, i)
                        str+= "            <newMode></newMode>\n        </command>\n\n"
                        f.write(str)
                        ...
                        str+= "            </help>\n"
                        str+= "            <funcName type=\"user\">cli_%s_%s_%s_%s_get_mem_%s</funcName>\n" % (soc, mode, regList[0]._regName, key, i)
                        str+= "            <newMode></newMode>\n        </command>\n"
                        f.write(str)
        ...
        <command type="normal">
                ...
                <funcName type="user">cli_%s_%s_exit</funcName>
                <newMode></newMode>
	</command>\n	
    </view>
</root>\n""" % (soc, mode.lower())
    ...
    def GenerateRegMemXMLFile_For_HASH_TCAM_divide_key_and_data(self, allObjDict, mode, soc):
        ...
        if 1 == 1:
            ...
            if 1 == 0: #mode == "st_l3_defip":
                self.GenerateRegMemXMLFile_For_TCAM_st_l3_defip(allObjDict, mode, soc)
                return
...
<root>"""
    ...
    <view name="%s_%s_MODE">\n""" %(soc, mode.upper()) # Here the mode name should be upper.
            ...
            if allObjDict[mode][2][0] > 0:
                ...
                # add commands for tables which have different KeyTypes             
                    ...
                    for info in fieldInfoList :
                        ...
                        str = "            </help>\n"
                        str+= "            <funcName type=\"user\">cli_%s_%s_%s_%s_set_mem</funcName>\n" % (soc, mode, regList[0]._regName, key)
                        str+= "            <newMode></newMode>\n        </command>\n\n"
                        f.write(str)
                        ...
                        str+= "            </help>\n"
                        str+= "            <funcName type=\"user\">cli_%s_%s_%s_%s_get_mem</funcName>\n" % (soc, mode, regList[0]._regName, key)
                        str+= "            <newMode></newMode>\n        </command>\n"
                        f.write(str)
            ...
            if allObjDict[mode][1][0] > 0:                   
                ...
                str+= "            </help>\n"
                str+= "            <funcName type=\"user\">cli_%s_%s_test_reg</funcName>\n" % (soc, mode)
                str+= "            <newMode></newMode>\n        </command>\n\n"                    
                f.write(str)
            ...
            if allObjDict[mode][1][0] > 0:                   
                ...
                str+= "                <helpInfo>The value set to the register then compare.</helpInfo>\n"
                str+= "            </help>\n"
                str+= "            <funcName type=\"user\">cli_%s_%s_i2c_test_reg</funcName>\n" % (soc, mode)
                str+= "            <newMode></newMode>\n        </command>\n\n"                    
                f.write(str)
            ...
            if allObjDict[mode][2][0] > 0:
                ...
                for reg in range(allObjDict[mode][2][0]):
                        ...
                        str+= "            </help>\n"
                        str+= "            <funcName type=\"user\">cli_%s_%s_%s_test_mem</funcName>\n" % (soc, mode, regList[reg]._regName)
                        str+= "            <newMode></newMode>\n        </command>\n"
                        f.write(str)
            ...
            if allObjDict[mode][2][0] > 0:
                ...
                for reg in range(allObjDict[mode][2][0]):
                        ...
                        str+= "                <helpInfo>The data value.</helpInfo>\n"                        
                        str+= "            </help>\n"
                        str+= "            <funcName type=\"user\">cli_%s_%s_%s_i2c_test_mem</funcName>\n" % (soc, mode, regList[reg]._regName)
                        str+= "            <newMode></newMode>\n        </command>\n"
                        f.write(str)
            ...
            if allObjDict[mode][2][0] > 0:
                ...
                for reg in range(allObjDict[mode][2][0]):
                        ...
                        str+= "            </help>\n"
                        str+= "            <funcName type=\"user\">cli_%s_%s_%s_dump_or_clean_mem</funcName>\n" % (soc, mode, regList[reg]._regName)
                        str+= "            <newMode></newMode>\n        </command>\n"
                        f.write(str)
    ...
    def GenerateRegMemXMLFile_For_TABLE_POOL(self, allObjDict, mode, soc):
...
<root>"""
    ...
    <view name="%s_%s_MODE">\n""" %(soc, mode.upper()) # Here the mode name should be upper.
            ...
            if allObjDict[mode][2][0] > 0:
                ...
                for reg in range(allObjDict[mode][2][0]):
                    ...
                    for info in fieldInfoList :
                        ...
                        str = "            </help>\n"
                        str+= "            <funcName type=\"user\">cli_%s_%s_%s_%s_set_mem</funcName>\n" % (soc, mode, regList[reg]._regName, key)
                        str+= "            <newMode></newMode>\n        </command>\n\n"
                        f.write(str)
            ...
            if mode.lower() in ["cb_cfg", "cb_cnt"]:                                   
                ...
                str+= "            </help>\n"
                str+= "            <funcName type=\"user\">cli_clean_all_%s_%s_reg_table</funcName>\n" % (soc, mode)
                str+= "            <newMode></newMode>\n        </command>\n\n"
                f.write(str)
...
#                f.write(str)
                ...
                if 1 == 1:                                    
                    ...
                    str = "            </help>\n"
                    str+= "            <funcName type=\"user\">cli_%s_bfd_status_set_mem</funcName>\n" % soc
                    str+= "            <newMode></newMode>\n        </command>\n\n"
                    f.write(str)
                    ...
                    str+= "                <helpInfo>The index value.</helpInfo>\n"
                    str+= "            </help>\n"
                    str+= "            <funcName type=\"user\">cli_%s_bfd_status_get_mem</funcName>\n" % soc
                    str+= "            <newMode></newMode>\n        </command>\n"
                    f.write(str)
    ...
    def GenerateRegMemInterfaces(self, allObjDict, soc):
        print "Generate the C Source Code File."
        

        f = file("%s_reg_mem_interface.c" % soc, "w")
        f.write("\n/* THE FILE IS GENERATED AUTOMATICLY, DO NOT EDIT BY MANNUAL!!!*/\n\n")
        ...
        for mode in allObjDict:
            if allObjDict[mode][1][0] > 0:
                ...
                for reg in range(len(regList)):
                    ...
                    str = "int cli_%s_%s_%s_set_reg(int module_id, int argc, char *args[])\n" % (soc, mode, regList[reg]._regName)
                    str += "{\n    return reg_set(module_id, argc, args, \"%s\");\n}\n\n" % regList[reg]._regName

                    #add get function for register
                    str += "int cli_%s_%s_%s_get_reg(int module_id, int argc, char *args[])\n" % (soc, mode, regList[reg]._regName)
                ...
                for mem in range(len(memList)):
                    ...
                    for info in fieldInfoList:                        
                        ...
                        for i in range(2):                        
                            ...
                            str = "int cli_%s_%s_%s_%s_set_mem_%s(int module_id, int argc, char *args[])\n" % (soc, mode, memList[mem]._regName, key, i)
                            str += "{\n    return mem_set(module_id, argc, args, \"%s\", %d, \"%s\");\n}\n\n" % (memList[mem]._regName, int(key), table_type)
    
                            #add get function for table
                            str += "int cli_%s_%s_%s_%s_get_mem_%s(int module_id, int argc, char *args[])\n" % (soc, mode, memList[mem]._regName, key, i)
            ...
            if allObjDict[mode][2][0] > 0:
                ...
                for mem in range(len(memList)):
                    ...
                    for info in fieldInfoList:                        
                        ...
                        #print "sheet %s \n" % mode
                        #add set function for table
                        str = "int cli_%s_%s_%s_%s_set_mem(int module_id, int argc, char *args[])\n" % (soc, mode, memList[mem]._regName, key)
                        str += "{\n    return mem_set(module_id, argc, args, \"%s\", %d, \"%s\");\n}\n\n" % (memList[mem]._regName, int(key), table_type)
    
                        #add get function for table
                        str += "int cli_%s_%s_%s_%s_get_mem(int module_id, int argc, char *args[])\n" % (soc, mode, memList[mem]._regName, key)
                        str += "{\n    return mem_get(module_id, argc, args, \"%s\", %d, \"%s\");\n}\n\n" % (memList[mem]._regName, int(key), table_type)
                        f.write(str)
            ...
            # add test commands for test all registers
            if allObjDict[mode][1][0] > 0:                
                str = "int cli_%s_%s_test_reg(int module_id, int argc, char *args[])\n" % (soc, mode)
                str += "{\n    return reg_test(module_id, argc, args);\n}\n\n"

                str += "int cli_%s_%s_i2c_test_reg(int module_id, int argc, char *args[])\n" % (soc, mode)
            ...
            if allObjDict[mode][2][0] > 0:
                ...
                memList = allObjDict[mode][2][1]
                for mem in range(len(memList)):
                        str = "int cli_%s_%s_%s_test_mem(int module_id, int argc, char *args[])\n" % (soc, mode, memList[mem]._regName)
                        str += "{\n    return mem_test(module_id, argc, args, \"%s\", \"%s\");\n}\n\n" % (memList[mem]._regName, table_type)
                        str += "int cli_%s_%s_%s_i2c_test_mem(int module_id, int argc, char *args[])\n" % (soc, mode, memList[mem]._regName)
                        str += "{\n    return mem_i2c_test(module_id, argc, args, \"%s\", \"%s\");\n}\n\n" % (memList[mem]._regName, table_type)
                        str += "int cli_%s_%s_%s_dump_or_clean_mem(int module_id, int argc, char *args[])\n" % (soc, mode, memList[mem]._regName)
            ...
            # add clean commands for all registers and tables in cb_cfg and cb_cnt mode
            if mode.lower() in ["cb_cfg", "cb_cnt"]:
                str = "int cli_clean_all_%s_%s_reg_table(int module_id, int argc, char *args[])\n" % (soc, mode)
...
############################################################################################################
    ...
    def GenerateAllTableStruct(self, objDict, soc):
        print "Generate the alltablestruct.h File."
        f = file(soc + "_" + 'allTableStruct.h', 'w')
        f.write("\n//THE FILE IS GENERATED AUTOMATICLY, DO NOT EDIT BY MANNUAL!!!\n\n")
    ...
    def GenerateAllTableAPI(self, objDict, soc):
        print "Generate the alltablestruct.h File."
        f = file(soc + "_" + 'allTableApi.c', 'w')
        f.write("\n//THE FILE IS GENERATED AUTOMATICLY, DO NOT EDIT BY MANNUAL!!!\n\n")
            ...
            if objDict[mode][2][0] > 0:
                ...
                for reg in range(len(regList)): # regList only have one table. (but the table maybe divided to key or data table.)
                    ...
                    if nKeyType > 1:
                        ...
                        struct_name = regList[reg]._table_struct_name.lower()
                        #print "%s - %s - %s " % ( regList[reg]._excelName, regList[reg]._sheetName , struct_name)
                        if struct_name == "100":
                            struct_name = regList[reg]._sheetName
                        
                        f.write("void soc_%s_add(%s_t tab, char *buff)\n{\n" %(struct_name, struct_name))
                                             
                        f.write("    printf(\"%s set api\");\n" % struct_name)
                        
                        str ="""
	field_info_t fields[%d];
	mem_info_t %s;
	%s.nFields = %d;
	%s.fields = fields;
	soc_mem_add(%s, buff);
}\n\n
""" % (len(fieldList), struct_name, struct_name, len(fieldList), struct_name, struct_name)
    ...
    def find_same_reg_in_different_mode_by_name(self, sameModeObjsDict, name, sameNameList):
    ...
    def find_same_reg_in_different_mode_by_name_2(self, sameModeObjsDict, name, sameNameList, i):
    ...
    def show_the_same_name_register_in_different_mode(self, sameModeObjsDict, soc):
        ...
        for mode in sameModeObjsDict:
            ...
            for reg in sameModeObjsDict[mode][1][1]:
                ...
                if reg._excelName.find("egress_table_csr") != -1 :                    
                    continue
                if reg._excelName.find("hirar_table_csr") != -1 :                    
                    continue
                if reg._excelName.find("ingress1_table_csr") != -1 :
                    continue
                if reg._excelName.find("ingress2_table_csr") != -1 :
                    continue
                if reg._excelName.find("ingress3_table_csr") != -1 :
                    continue                            
                if reg._excelName.find("tcam_wrapper_CSR") != -1 :
                    continue
                
                #modes = self.find_same_reg_in_different_mode_by_name(sameModeObjsDict, reg._regName, sameNameList)
                modes = self.find_same_reg_in_different_mode_by_name_2(sameModeObjsDict, reg._regName, sameNameList, i)
                
                
                if len(modes) > 1:                    
                    item = [modes, reg._regName]                    
                    sameNameList.append(item)                    

        f = file(soc + "_" + 'same_name_reg_in_different_mode', 'w')
        
        
        for i in range(100):
            print "########## Register in %d modes\n" % i
